...
namespace AudioEffects {
    ...
    void BitCrush(uint16_t* sampleBuffer, int samples, float quant, float gainFactor) {
        ...
        const float volumeReductionFactor = 0.3f;  // Reduce overall volume for softer output
        ...
        for (int i = 0; i < samples; i++) {
            ...
            // Reduce volume and re-quantize to 16-bit range
            sampleBuffer[i] = static_cast<uint16_t>(std::clamp(filteredSample * volumeReductionFactor * 32768.0f + 32768.0f, 0.0f, 65535.0f));
        }
    }
